`hsTableReader` <-
  function(con="",cols='character',chunkSize=-1,FUN=print, ignoreKey=TRUE,singleKey=TRUE, skip=0, sep='\t', keyCol='key', PFUN=NULL,carryMemLimit=512e6,carryMaxRows=Inf,stringsAsFactors=FALSE,debug=FALSE,naStrings=c()) {
    ...
    if (skip > 0) {
      junk = scan(con,what='character',quiet=!debug,sep=sep,nlines=skip,quote="",na.strings=naStrings)
    }
    
    if (ignoreKey) {
      repeat {
         a = scan(con,what=cols,quiet=!debug,sep=sep,strip.white=TRUE,nlines=chunkSize,flush=TRUE,quote="",na.strings=naStrings)
    ...
    if (is.null(PFUN)) {
      ...
      repeat {
        ...
        ## Memory Report
        if (debug) {
          hsWriteTable(data.frame(a),stdout())
